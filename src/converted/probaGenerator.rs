...
*  Local variables
**************************************/
static mut g_programName: *mut libc::c_char =
...
unsafe extern "C" fn usage() -> libc::c_int {
    ...
    fprintf(__stderrp, b"%s P%%\n\x00" as *const u8 as *const libc::c_char,
            g_programName);
    fprintf(__stderrp,
            b"Exemple :\n\x00" as *const u8 as *const libc::c_char);
    fprintf(__stderrp, b"%s 70%%\n\x00" as *const u8 as *const libc::c_char,
            g_programName);
    return 0i32;
}
...
unsafe extern "C" fn generate(mut buffer: *mut libc::c_void,
    ...
    if p == 0.0f64 { p = 0.005f64 }
    fprintf(__stderrp,
            b"Generating %u KB with P=%.2f%%\n\x00" as *const u8 as
                *const libc::c_char, (buffSize >> 10i32) as libc::c_uint,
    ...
    while op < oend {
        let r: libc::c_uint =
            GEN_rand(&mut seed) & (4096i32 - 1i32) as libc::c_uint;
        let fresh1 = op;
        op = op.offset(1);
...
#[no_mangle]
pub unsafe extern "C" fn createSampleFile(mut filename: *mut libc::c_char,
                                          mut p: libc::c_double) {
    let foutput: *mut FILE =
        fopen(filename, b"wb\x00" as *const u8 as *const libc::c_char);
    if foutput.is_null() {
        perror(b"dataGenerator:\x00" as *const u8 as *const libc::c_char);
        exit(1i32);
    }
    let buffer: *mut libc::c_void =
        malloc((1i32 * (1i32 << 20i32) - 1i32) as libc::c_ulong);
    generate(buffer, (1i32 * (1i32 << 20i32) - 1i32) as size_t, p);
    fwrite(buffer, 1i32 as libc::c_ulong,
           (1i32 * (1i32 << 20i32) - 1i32) as libc::c_ulong, foutput);
    free(buffer);
    fclose(foutput);
    fprintf(__stderrp,
            b"File %s generated\n\x00" as *const u8 as *const libc::c_char,
 ...
 -> libc::c_int {
    ...
    let mut filename: [libc::c_char; 10] =
        *::std::mem::transmute::<&[u8; 10],
                                 &mut [libc::c_char; 10]>(b"proba.bin\x00");
    g_programName = *argv.offset(0isize);
    fprintf(__stderrp,
            b"Binary file generator\n\x00" as *const u8 as
                *const libc::c_char);
    if argc < 2i32 { badusage(); }
